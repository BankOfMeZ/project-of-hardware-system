...
int x, y, myCount;//x表示当前局面○三子一线的数目，y表示当前局面×三子一线的数目，myCount表示当前○和×的总数
...
int isWin() { //1表示电脑获得胜利，-1表示玩家获得胜利，0表示尚未分出胜负
	int i;
	for (i = 0; i < 3; i++)
	{
...
void PrintChessboard()
{
	...
	for (i = 0; i < 3; i++)
	{
		for (j = 0; j < 3; j++)
		{
			if (chess[i][j] == 1) showChessPieces(1); // printf("○");
			else if (chess[i][j] == 0) showChessPieces(0); // printf("□");
			else if (chess[i][j] == -1) showChessPieces(-1); // printf("×");
		}
		// cout << "" << endl;
	}
}
...
int PlayTicTacToe()
{
	...
	while(1)
	{
		...
		if(option == 2 || player == 1) PlayerInput(player);
		else PcPlay(); 
